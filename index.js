...
import {
  Text, Animated, Dimensions, View, Image,
  ImageBackground,
...
export default class AnimationFlatlist extends React.Component {
  ...
  render() {
    ...
    return (
      <SafeAreaView style={[styles.container, { backgroundColor: this.props.secondaryBackgroundColor }]}>
        <Animated.View style={{
          position: 'absolute', top: 0, left: 0, bottom: 0, right: width / 3, backgroundColor: this.props.primaryBackgroundColor,
  ...
  renderItem = ({ item, index }) => {
    ...
    return (
      <Animated.View key={index + 1 + 'll'} style={{
        marginLeft: (index == 0 ? (width - ITEM_SIZE) / 2 : 0), marginRight: (index == this.props.data.length - 1 ? (width - ITEM_SIZE) / 2 : 0), marginHorizontal: 10, height: this.props.height, borderRadius: 8, overflow: 'hidden', alignSelf: 'center', width: ITEM_SIZE, transform: [{ scaleX: translateY }, { scaleY: translateY }],
        overflow: 'hidden'
      }}>
        ...
        <TouchableOpacity
          onPress={() => this.props.onPressDetail({ item, index })}
          style={{ ...styles.btnDetail, ...this.props.styleButton }}>
          {this.props.childrenButton}
        </TouchableOpacity>
      </Animated.View>
    )
...
const styles = StyleSheet.create({
  btnDetail: {
    // backgroundColor: '#F0076C', width: 50, height: 50, borderRadius: 50 / 2,
